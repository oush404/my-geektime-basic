...
type GORMMAsyncSmsDAO struct {
	db *gorm.DB
}

func (g *GORMMAsyncSmsDAO) Insert(ctx context.Context, s AsyncSms) error {
	return g.db.Create(&s).Error
}

func (g *GORMMAsyncSmsDAO) GetWaitingSMS(ctx context.Context) (AsyncSms, error) {
...
func (g *GORMMAsyncSmsDAO) MarkSuccess(ctx context.Context, id int64) error {
	now := time.Now().UnixMilli()
...
func (g *GORMMAsyncSmsDAO) MarkFailed(ctx context.Context, id int64) error {
	now := time.Now().UnixMilli()
	return g.db.WithContext(ctx).Model(&AsyncSms{}).
		// 只有到达了重试次数才会更新
